/**
 * TMS (tms.esimson.com) roster scrape — local Node only.
 *
 * Credentials from env (never send to the browser):
...
function getConfig() {
    const baseUrl = String(process.env.TMS_BASE_URL || DEFAULT_BASE)
        .trim()
        .replace(/\/$/, '');
    const username = String(process.env.TMS_USERNAME || '').trim();
    const password = String(process.env.TMS_PASSWORD || '');
    const rosterUrls = String(process.env.TMS_ROSTER_URLS || '')
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean)
    ...
    return new Promise((resolve, reject) => {
        const req = lib.request(
            ...
            (res) => {
                ...
                res.on('end', () => {
                    if (opts.jar) {
                        storeCookies(opts.jar, res.headers['set-cookie']);
                    }
...
function parseStudentsFromNumberedBlocks(text) {
    ...
    while (i < lines.length) {
        ...
        // Numbered line is location / shooting tag — never the Korean name.
        let name = '';
        let nameEn = '';
        let j = i + 1;
...
function parseStudentsFromClassPopup(html) {
    ...
    while ((m = re.exec(raw))) {
        ...
        const name = stripTags(decodeHtmlEntities(m[2])).replace(/\s+/g, ' ').trim();
