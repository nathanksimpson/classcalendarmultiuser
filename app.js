...
function buildSyllabusTimelineForClass(classData) {
    ...
    const effective = scheduleOpts.effectiveClassData;
    const schedule = calculateLessonDates(classData, scheduleOpts);
    ...
    if (classData.customSchedule && classData.customSchedule.enabled) {
        const slots = lessons.map(lesson => {
            ...
            const isHol = isHolidayForClass(dateStr, effective);
            return {
                date: dateStr,
                monthKey,
...
function ensureUiState() {
    ...
    if (appData.ui.showAllClassCurricula === undefined) {
        appData.ui.showAllClassCurricula = false;
    }
        ...
        getClassCurriculumSlicesForCalendar(classData).forEach((slice) => {
            ...
            lessons.forEach(lesson => {
                ...
                scheduledLessons[dateStr].push({
                    ...
                    lesson,
                    slice,
                    calendarTitle: slice.calendarTitle || classData.name
                });
            });
        });
    });
...
async function flushTeamSaveAfterDayNotesChange() {
    if (!teamSyncEnabled || typeof CalendarSync === 'undefined' || !CalendarSync.flushPendingSave) {
        return;
    }
...
function initClassNotesPanelListeners() {
    ...
    shell.querySelector('#classNotesFilterFrom')?.addEventListener('change', () => {
        renderClassNotesTab();
        saveClassNotesFiltersToUi();
    });
    shell.querySelector('#classNotesFilterTo')?.addEventListener('change', () => {
        renderClassNotesTab();
        saveClassNotesFiltersToUi();
    });
    ...
    shell.querySelector('#classNotesFilterClearClassesBtn')?.addEventListener('click', () => {
        shell.querySelectorAll(`input[${CLASS_NOTES_FILTER_ATTR}="classIds"]`).forEach((input) => {
            input.checked = false;
        });
        populateClassNotesAddClassSelect();
...
function populateSyllabusEditorForClass(classData) {
    ...
    if (syllabusHint) {
        const viewer = getCalendarViewerTeacherSelector();
        syllabusHint.textContent = t('syllabusViewingCurriculum');
        syllabusHint.hidden = !viewer;
    }
...
function selectClassSuggestion(classId) {
    ...
    if (existingClass) {
        ...
        if (existingClass.useAutoTermEnd === true && elements.classStartDate.value) {
            ...
            const draft = { ...existingClass };
            if (classUsesDebateCompression(draft)) {
                ensureDebatePeriodsOnClass(draft);
                renderDebateBookPeriodRows(draft.debateBookPeriods || []);
            } else {
...
// Class Management
// ============================================
function validateClassTeacherAccountsBeforeSave() {
    const mount = document.getElementById('classTeachersRows');
    if (!mount) {
        return true;
    }
    ...
    mount.querySelectorAll('.class-teacher-row').forEach((rowEl) => {
        ...
        const nameInput = rowEl.querySelector('.class-teacher-row-name');
        const curriculumSel = rowEl.querySelector('.class-teacher-row-curriculum');
        const parsed = parseTeacherPickerValue(sel && sel.value);
        const name = (nameInput && nameInput.value ? nameInput.value : '').trim();
        const curriculumId = (curriculumSel && curriculumSel.value ? curriculumSel.value : '').trim();
        if ((name || curriculumId || parsed.displayName) && !parsed.userId) {
            needsAccount = true;
        }
    });
    if (needsAccount) {
        alert(t('classTeacherAccountRequired'));
        return false;
    }
    return true;
}
...
function handleClassSubmit(e) {
    ...
    if (!validateClassTeacherAccountsBeforeSave()) {
        return;
    }
...
function getCalendarTileTypographyConfig(mode) {
    return mode === 'print' ? CALENDAR_TILE_TYPO.print : CALENDAR_TILE_TYPO.screen;
}

function resolveCalendarTileTypographyRoot(doc, root) {
    if (!root) {
        return null;
    }
...
function clearCalendarTileTypography(doc, root) {
    const scope = resolveCalendarTileTypographyRoot(doc, root);
    if (!scope) {
        return;
    }
...
/**
 * Scale class, holiday, and event tile fonts to fill each day cell.
 * @param {Document} doc
 * @param {Element|string} root
 * @param {{ mode?: 'screen'|'print' }} [options]
 */
function fitCalendarTileTypography(doc, root, options = {}) {
    const scope = resolveCalendarTileTypographyRoot(doc, root);
    if (!scope) {
        return;
    }
...
function setupCalendarTileTypographyObserver() {
    if (calendarTileTypographyResizeObserver || !elements.calendarContainer) {
        return;
    }
    if (typeof ResizeObserver === 'undefined') {
        return;
    ...
    calendarTileTypographyResizeObserver = new ResizeObserver(() => {
        ...
        calendarTileTypographyResizeTimer = setTimeout(() => {
            calendarTileTypographyResizeTimer = null;
            scheduleCalendarTileTypographyFit();
        }, 100);
    });
    calendarTileTypographyResizeObserver.observe(elements.calendarContainer);
}
...
function migrateData(data) {
    ...
    if (data.classes && data.classes.length > 0) {
        data.classes.forEach(classData => {
            ...
            if (classData.assignedTeacherName === undefined) {
                classData.assignedTeacherName = '';
            }
            if (classData.teacherCategory === undefined) {
                classData.teacherCategory = '';
            }
            ...
            classData.classTeachers.forEach((row) => {
                ...
                if (row.curriculumId === undefined) {
                    row.curriculumId = classData.curriculumId || '';
                    migrated = true;
                }
                if (row.classTypeId === undefined) {
                    row.classTypeId = classData.classTypeId || '';
                    migrated = true;
                }
            ...
            if (classData.homeroomTeacherUserId === undefined) {
                classData.homeroomTeacherUserId = '';
            }
            if (classData.homeroomTeacherName === undefined) {
                classData.homeroomTeacherName = '';
            }
            if (!classData.scheduleBlock) {
                classData.scheduleBlock = 'primary';
            }
            if (classData.timeSlotId === undefined) {
                classData.timeSlotId = '';
            }
        });
    }
    ...
    if (Array.isArray(data.cohorts)) {
        data.cohorts.forEach((cohort) => {
            ...
            if (cohort.homeroomTeacherUserId === undefined) {
                cohort.homeroomTeacherUserId = '';
            }
            if (cohort.homeroomTeacherName === undefined) {
                cohort.homeroomTeacherName = '';
            }
            if (cohort.homeroomDaySuffix === undefined) {
                cohort.homeroomDaySuffix = '';
            }
