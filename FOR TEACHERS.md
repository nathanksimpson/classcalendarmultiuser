# For teachers — Class Calendar (team)

Quick guide for daily use. For full planner features (tabs, syllabus, print), open **Help** in the app after you sign in.
...
1. Open your bookmark in the browser (not `index.html` from a folder).
2. Click **Login with Kakao** and approve if asked.
...
## Editing with colleagues (edit lock)

- Click **Start editing** when no one else is editing.
- Click **Release lock** when you are done.
- If someone else is editing, you are read-only — click **Request to edit** and wait.
- The current editor can **Allow** or **Dismiss** your request.
- There is **no** “take over” button — only request, allow, release, or wait for an inactive lock to expire.
...
- Run any `.bat` file or server
- Open files from a shared folder
- Know what "localhost" means
