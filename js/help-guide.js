...
(function (global) {
    ...
    "en": [
        ...
        "View calendars (global) — open team calendars you are assigned to on Admin → Calendars. View all calendars — open any team calendar without a separate assignment (super admin preset only).",
        ...
        "Manage calendar access / delete: teachers only manage calendars they created (+ New). Head teachers also get Manage groups and Manage calendar access (any team calendar on Admin → Calendars). Super admins (or anyone with Delete calendars / View all calendars checkboxes) can delete or open every calendar without assignment.",
    ...
    "ko": [
        ...
        "캘린더 보기(전역) — Admin → Calendars에서 지정한 팀 캘린더를 엽니다. 모든 캘린더 보기 — 별도 지정 없이 모든 팀 캘린더를 엽니다(최고 관리자 프리셋만).",
        "캘린더 접근 수준 — Admin → Calendars 탭에서 캘린더마다: Editor(팀 잠금 하에 편집), Suggester(제안; 편집자·담당 선생님이 적용), Viewer(해당 캘린더 읽기·인쇄만).",
        "캘린더 접근 관리/삭제: 선생님은 본인이 만든 캘린더(+ New)만 관리합니다. Head teacher는 그룹 관리·캘린더 접근 관리(Admin → Calendars의 모든 팀 캘린더)도 됩니다. 최고 관리자 또는 Delete calendars / View all calendars 권한이 있으면 지정 없이 모든 캘린더를 열거나 삭제할 수 있습니다.",
